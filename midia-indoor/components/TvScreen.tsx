...
export default function TvScreen({ token, device }: { token: string; device: string }) {
  ...
  useEffect(() => {
    if (!data?.birthdays.length) return;
    const timer = setInterval(() => setBirthdayIndex((index) => (index + 1) % data.birthdays.length), data.settings.birthdayDurationSec * 1000);
  ...
  const birthday = data?.birthdays[birthdayIndex % Math.max(data.birthdays.length, 1)];
  ...
  return <main className="relative h-screen w-screen overflow-hidden bg-[#080a0d] p-[1.4vw]">
    <div className="grid h-full grid-cols-[minmax(0,3.2fr)_minmax(280px,1fr)] grid-rows-[1fr_auto] gap-[1.2vw]">
      <section className="relative overflow-hidden rounded-[1.3vw] border border-white/10 bg-[#0d1116] shadow-2xl">
        ...
        {welcome && <div className="welcome-enter absolute inset-x-[3.5vw] bottom-[3vw] z-20 overflow-hidden rounded-[1.2vw] border border-blue-300/30 bg-[linear-gradient(115deg,rgba(17,71,155,.97),rgba(38,126,255,.92))] px-[3.2vw] py-[2.1vw] shadow-[0_28px_80px_rgba(22,91,205,.45)] backdrop-blur-xl">
      ...
      <aside className="flex min-h-0 flex-col overflow-hidden rounded-[1.3vw] border border-amber-200/15 bg-[radial-gradient(circle_at_top,#282312_0%,#13140f_45%,#0d1011_100%)] p-[1.6vw] shadow-2xl">
        <div className="flex items-center gap-[.65vw]"><span className="h-[.55vw] w-[.55vw] rounded-full bg-[#f1c85b] shadow-[0_0_18px_#f1c85b]"/><p className="text-[.88vw] font-extrabold uppercase tracking-[.22em] text-[#f1c85b]">Aniversariantes do dia</p></div>
        <div className="my-[1.4vw] h-px bg-gradient-to-r from-[#f1c85b]/55 to-transparent"/>
        <div key={birthday?.id || "none"} className="birthday-enter flex flex-1 flex-col items-center justify-center text-center">
          {birthday ? <><div className="grid h-[10vw] w-[10vw] place-items-center overflow-hidden rounded-full border-[.3vw] border-[#f1c85b]/70 bg-[#25281f] shadow-[0_0_50px_rgba(241,200,91,.15)]">{birthday.photoUrl ? <img src={birthday.photoUrl} alt="" className="h-full w-full object-cover"/> : <span className="text-[3.5vw] font-black text-[#f1c85b]">{birthday.name.charAt(0)}</span>}</div><p className="mt-[1.4vw] text-[2.05vw] font-black leading-tight">{displayBirthdayName(birthday)}</p><p className="mt-[.8vw] max-w-[18vw] text-[1.1vw] leading-relaxed text-white/62">{birthday.message || `Hoje é dia de celebrar você. Parabéns, ${birthday.name.split(" ")[0]}!`}</p></> : <><div className="grid h-[8vw] w-[8vw] place-items-center rounded-full border border-[#f1c85b]/25 bg-[#f1c85b]/5 text-[3vw]">✦</div><p className="mt-[1.5vw] text-[1.5vw] font-bold leading-snug">Hoje não temos aniversariantes.</p><p className="mt-[.8vw] text-[1vw] leading-relaxed text-white/55">Um excelente treino para todos!</p></>}
        </div>
        {data.birthdays.length > 1 && <div className="mt-auto flex justify-center gap-2">{data.birthdays.map((item, index) => <span key={item.id} className={`h-1 rounded-full transition-all ${index === birthdayIndex ? "w-8 bg-[#f1c85b]" : "w-2 bg-white/20"}`}/>)}</div>}
      </aside>

      <footer className="col-span-2 flex items-center justify-between px-[.4vw]">
